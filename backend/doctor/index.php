                                ...
                                datasets: [{
                                    ...
                                    data: [
                                        ...
                                        foreach ($month_array as $key => $month_value) {
                                            $result = mysqli_num_rows(mysqli_query($connection, "SELECT * FROM appointment WHERE MONTH(app_date) = '" . ++$key . "' AND doctor_id = '" . $doctor_row['doctor_id'] . "' AND consult_status = 1 "));
                                            echo "$result,";
                                        }
                                        ?>
                                    ],
                        ...
                        var myChart = new Chart(ctx, {
                            ...
                            data: {
                                ...
                                labels: [
                                    <?php
                                    $idquery = array();
                                    $result = mysqli_query($connection, "SELECT DISTINCT patient_nationality FROM appointment INNER JOIN patients ON appointment.patient_id = patients.patient_id WHERE doctor_id = " . $doctor_row['doctor_id'] . " ");
                            ...
                            options: {
                                title: {
                                    display: true,
                                    text: 'Appointment Visited Based on Nationality',
                                }
                            }
                        });
                    </script>
                        ...
                        var myChart = new Chart(ctx, {
                            ...
                            data: {
                                ...
                                labels: [
                                    <?php
                                    $idquery = array();
                                    $result = mysqli_query($connection, "SELECT DISTINCT app_time FROM appointment WHERE doctor_id = " . $doctor_row['doctor_id'] . " ORDER BY app_time DESC");
                                    while ($row = mysqli_fetch_assoc($result)) {
                                        echo '"' . $row['app_time'] . '",';
