...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $forgotstmt = $connection->prepare("SELECT * FROM doctors WHERE doctor_email = ?");
...
if (isset($_POST['forgotbtn'])) {
    if (count($errors) == 0) {
        ...
        $link = $_SERVER["SERVER_NAME"] . "/lifecare/backend/doctor/reset.php?selector=" . $selector . "&validator=" . bin2hex($validator);
        $expries = date("U") + 1800;

        $userEmail = $_POST["inputEmailAddress"];

        $statement = $connection->prepare("DELETE FROM doctor_reset WHERE reset_email = ?");
        $statement->bind_param("s", $userEmail);
        $statement->execute();

        $hashedToken = password_hash($validator, PASSWORD_DEFAULT);

        $statement = $connection->prepare("INSERT INTO doctor_reset (reset_email, reset_selector, reset_token, reset_expires) VALUE (?,?,?,?)");
