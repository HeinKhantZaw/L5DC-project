...
$result = $connection->query("SELECT * FROM appointment LEFT JOIN patients ON appointment.patient_id = patients.patient_id WHERE appointment.app_id = $app_id");
...
if (isset($_POST['prescriptionbtn'])) {
    ...
    if (count($errors) == 0) {
        $statement = $connection->prepare("INSERT INTO medical_record (med_sympton, med_diagnosis, med_advice, med_date, patient_id, clinic_id, doctor_id) VALUE (?,?,?,?,?,?,?) ");
        $statement->bind_param("sssssss", $sympton, $diagnosis, $advice, $date_created, $patient_id, $doctor_row['clinic_id'], $doctor_row['doctor_id']);
        $statement->execute();
        $statement->close();
...
if (isset($_POST['appointmentbtn'])) {
    ...
    $treatment = $connection->real_escape_string($_POST['inputTreatment']);
    ...
    if (count($apperrors) == 0) {
        $appstmt = $connection->prepare("INSERT INTO appointment (app_date, app_time, treatment_type, patient_id, clinic_id, doctor_id) VALUE (?,?,?,?,?,?) ");
...
if (isset($_POST["completebtn"])) {
    ...
    $comstmt = $connection->prepare("UPDATE appointment SET consult_status = 1 WHERE app_id = ? ");
                ...
                <div class="modal-dialog modal-lg" role="document">
                    <div class="modal-content">
                        <div class="modal-header">
                            <h6 class="modal-title">Add
                                <strong><?= $row["patient_firstname"] . ' ' . $row["patient_lastname"] ?></strong>
                                Follow Up Visit</h6>
                            ...
                            <div class="modal-body">
                                <div class="form-group">
                                    ...
                                    <select name="inputTreatment" id="inputTreatment" class="form-control">
                                        <?php
                                        $treatresult = mysqli_query($connection, "SELECT * FROM treatment_type WHERE doctor_id = '" . $doctor_row['doctor_id'] . "'");
                                ...
                                <div class="form-group">
                                    <input type="hidden" class="form-control form-control-sm"
                                           name="inputAppointmentDate" id="inputAppointmentDate">
                                    <input type="hidden" class="form-control form-control-sm"
                                           name="inputAppointmentTime" id="inputAppointmentTime">
                                </div>
                                <div class="form-row">
                        ...
                        <form action="<?= htmlspecialchars($_SERVER['REQUEST_URI']) ?>" method="POST">
                            ...
                            <div class="modal-body">
                                <div class="form-group">
                                    ...
                                    <textarea name="sympton" class="form-control" id="sympton" cols="30"
                                              rows="3"></textarea>
                                </div>
                                <div class="form-group">
                                ...
                                <div class="form-group">
                                    <label>Advice</label>
                                    <textarea name="advice" class="form-control" id="advice" cols="30"
                                              rows="3"></textarea>
                                </div>
                            </div>
                            <div class="modal-footer">
                ...
                <div class="modal-content">
                    ...
                    <form action="<?= htmlspecialchars($_SERVER['REQUEST_URI']); ?>" method="POST">
                        <div class="modal-body">
                            ...
                            Case Complete for <b><?= $row["patient_firstname"] . ' ' . $row["patient_lastname"] ?></b>
                        </div>
                ...
                <div class="card-body">
                    <div class="d-flex bd-highlight">
                        <div class="flex-fill bd-highlight">
                            ...
                            <h5 class="font-weight-bold"><?php echo $row["patient_firstname"] . ' ' . $row["patient_lastname"] ?></h5>
        ...
        <div class="col-md-12 mb-3">
            <nav class="navbar px-0 mb-3">
                <div class="nav nav-pills mr-auto">
                    <a class="nav-item text-sm-center nav-link active" data-toggle="pill" href="#tab1">Prescription
                        Info</a>
                    <a class="nav-item text-sm-center nav-link" data-toggle="pill" href="#tab3">Appointment Record</a>
                </div>
                <div class=" nav nav-pills ml-auto">
                    <a class="nav-item btn btn-sm btn-link" data-toggle="modal" href="#prescription">Add
                        Prescription</a>
                    <a class="nav-item btn btn-sm btn-link" data-toggle="modal" href="#followup">Add Appointment</a>
                    <button class="nav-item btn btn-sm btn-success" data-toggle="modal" href="#complete">Case Complete
                    </button>
                </div>
            </nav>
                    ...
                    <div class="card">
                        <div class="card-body">
                            <table class="table nowrap">
                                ...
                                <tbody>
                                <?php
                                $tresult = $connection->query("SELECT * FROM medical_record WHERE patient_id = $patient_id");
                                        ...
                                        <tr>
                                            ...
                                            <td>
                                                <button data-toggle="modal"
                                                        data-target="#viewdiagnosis<?= $trow['med_id'] ?>"
                                                        class="btn btn-sm btn-primary px-3">View
                                                </button>
                                            </td>
                                        </tr>

                                        <div class="modal fade" id="viewdiagnosis<?= $trow['med_id'] ?>" tabindex="-1"
                                             role="dialog">
                                            <div class="modal-dialog" role="document">
                                                <div class="modal-content">
                                                    <div class="modal-header">
                                                    ...
                                                    <div class="modal-body">
                                                        <div class="row">
                                                            <p class="col-sm-3 text-right"><b>Symptoms</b></p>
                                                            <div class="col-sm-6">
                    ...
                    <div class="card">
                        <div class="card-body">
                            <table class="table nowrap">
                                ...
                                <tbody>
                                <?php
                                $tresult = $connection->query("SELECT * FROM appointment WHERE patient_id = $patient_id ORDER BY app_date DESC");
                                if ($tresult->num_rows == 0) {
