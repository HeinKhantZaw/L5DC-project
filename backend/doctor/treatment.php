...
if (isset($_POST['submitbtn'])) {
    ...
    if ($errName == "") {
        $treatstmt = $connection->prepare("INSERT INTO treatment_type (treatment_name, doctor_id) VALUES (?, ?)");
...
if (isset($_POST['editbtn'])) {
    ...
    if ($modalerrName == "") {
        $treatstmt = $connection->prepare("UPDATE treatment_type SET treatment_name= ? WHERE treatment_id = ?");
...
if (isset($_POST['deletebtn'])) {
    $tid = escape_input($_POST['treatmentID']);
    $treatstmt = $connection->prepare("DELETE FROM treatment_type WHERE treatment_id = ?");
                ...
                <div class="card-body">
                    <form class="inline-form" action="<?= htmlspecialchars($_SERVER['PHP_SELF']); ?>" method="POST">
                        <div class="form-group row">
                            <label for="inputTreatment" class="col-sm-3 col-form-label text-right">Medical
                                Conditions</label>
                            <div class="col-sm-6">
                                <input type="text" name="inputTreatment" id="inputTreatment"
            ...
            <div class="card">
                <div class="card-body">
                    <table class="table">
                        ...
                        <tbody>
                        <?php
                        $tresult = $connection->query("SELECT * FROM treatment_type WHERE doctor_id = '" . $doctor_row['doctor_id'] . "' ");
                        if ($tresult->num_rows == 0) {
                            ...
                            while ($trow = $tresult->fetch_assoc()) {
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <a class="btn btn-sm btn-outline-info" data-toggle="modal"
                                           href="#edit<?= $trow['treatment_id'] ?>">Edit</a>
                                        <a class="btn btn-sm btn-outline-danger" data-toggle="modal"
                                           href="#delete<?= $trow['treatment_id'] ?>">Delete</a>
                                    </td>
                                </tr>

                                <div class="modal fade" id="edit<?= $trow['treatment_id'] ?>" tabindex="-1"
                                            ...
                                            <form action="<?= htmlspecialchars($_SERVER['PHP_SELF']); ?>" method="POST">
                                                <div class="modal-body">
                                                    <input type="hidden" name="treatmentID"
                                                           value="<?= $trow['treatment_id'] ?>">
                                                    <div class="form-group">
                                                        <input type="text" name="inputNewTreatment"
                                                               id="inputNewTreatment"
                                                               class="form-control <?= $modalclassName ?>"
                                                               value="<?= $trow['treatment_name'] ?>">
                                                        <?= $modalerrName ?>
                                                    </div>
                                                </div>
                                                <div class="modal-footer" style="border:none;">
                                                    ...
                                                    <button type="submit" name="editbtn" class="btn btn-sm btn-primary">
                                                        Save
                                                    </button>
                                                </div>
                                            </form>
                                ...
                                <div class="modal fade" id="delete<?= $trow['treatment_id'] ?>" tabindex="-1"
                                     role="dialog" aria-hidden="true">
                                    <div class="modal-dialog" role="document">
                                        <div class="modal-content">
                                            <div class="modal-header" style="border:none;">
                                            ...
                                            <form action="<?= htmlspecialchars($_SERVER['PHP_SELF']); ?>" method="POST">
                                                <div class="modal-body">
                                                    ...
                                                    <strong><?= $trow['treatment_name'] ?></strong> ?
                                                </div>
                                                <div class="modal-footer" style="border:none;">
                                                    <button type="button" class="btn btn-sm btn-secondary"
                                                            data-dismiss="modal">Close
