...
if ($result->num_rows == 0) {
    ...
    while ($row = $result->fetch_assoc()) {
        ...
        if ($dayofweek == $day) {
            ...
            ?>
            <button type="button" class="btn btn-sm btn-outline-primary" data-toggle="button" aria-pressed="false"
                    autocomplete="off" onClick="return getTime('<?= $timeslot ?>');"><?= $timeslot ?></button>
            <?php
        }
    }
}