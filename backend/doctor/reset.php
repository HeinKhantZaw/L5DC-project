...
if (ctype_xdigit($_GET["selector"]) !== false && ctype_xdigit($_GET["validator"]) !== false) {
    ...
    $check = $connection->prepare("SELECT * FROM doctor_reset WHERE reset_selector = ?");
...
/* if (isset($_GET['id'])) {
	$id = $_GET['id'];
	$result = $connection->query("SELECT doctor_email, doctor_token FROM doctors WHERE doctor_id = $id");
...
/* if (isset($_POST["resetbtn"])) {
	...
	if (count($errors) == 0) {
		$resetstmt = $connection->prepare("UPDATE doctors SET doctor_password = ? WHERE doctor_id = ?");
...
if (isset($_POST['resetbtn'])) {
    ...
    if (count($errors) == 0) {
        $currenDate = date("U");
        $statement = $connection->prepare("SELECT * FROM doctor_reset WHERE reset_selector = ? AND reset_expires >= ?");
        $statement->bind_param("ss", $selector, $currenDate);
        $statement->execute();
        ...
        if (!$row = $result->fetch_assoc()) {
            ...
            if ($tokenCheck === false) {
                ...
                $statement->bind_param("s", $tokenEmail);
                $statement->execute();
                $result = $statement->get_result();
                if (!$row = $result->fetch_assoc()) {
                    echo '<script>Swal.fire({title: "Error", text: "Resubmit Reset Request!", type: "error"}).then((result) => {
                    ...
                    $updatestmt = $connection->prepare("UPDATE doctors SET doctor_password = ?, doctor_token = ? WHERE doctor_email = ?");
