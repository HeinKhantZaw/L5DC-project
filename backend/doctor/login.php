...
if (array_key_exists("doctorlogin", $_SESSION) && $_SESSION["doctorlogin"] == 1) {
...
if (isset($_POST['loginbtn'])) {
    ...
    $check = $connection->prepare("SELECT * FROM doctors WHERE doctor_email = ? ");
    $check->bind_param("s", $inputEmail);
    $check->execute();
    $q = $check->get_result();
    $r = $q->fetch_assoc();
    if (mysqli_num_rows($q) != 1) {
        echo "<script>Swal.fire({title: 'Error!', text: 'Email does not Exist', type: 'error', confirmButtonText: 'Try Again'})</script>";
        exit();
    } else {
    ...
    try {
        $statement = $connection->prepare("SELECT * FROM doctors WHERE doctor_email = ? AND doctor_password = ?");
