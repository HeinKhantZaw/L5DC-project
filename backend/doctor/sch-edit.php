...
$id = $_GET['scheduleid'];
$schedulestmt = $connection->prepare("SELECT * FROM schedule WHERE schedule_id = ?");
...
if (isset($_POST['submitbtn'])) {
    ...
    if ($time != "" && $duration != "") {
        ...
        $statement = $connection->prepare("INSERT INTO schedule_detail (time_slot, duration, status, schedule_id) VALUE (?,?,?,?)");
        $statement->bind_param("ssss", $time, $duration, $status, $id);
        $statement->execute();
        $statement->close();
                    ...
                    <div class="card-body">
                        <form name="frm" method="POST"
                            ...
                            <div class="form-group">
                                ...
                                <input type="text" name="inputTime" class="form-control timepicker" id="inputTime"
                                       placeholder="From">
                            </div>
                            <div class="form-row">
                                <div class="form-group col-md-6">
                                    <label for="inputDuration">Duration</label>
                                    <select name="inputDuration" class="form-control <?= $classMin ?>"
                                            id="inputDuration">
                                        <?php
                                        for ($i = 1; $i <= 6; $i++) {
                                            if ($i > 1) {
                    ...
                    <div class="card-body">
                        <div class="data-tables">
                            <table id="datatable2" class="table table-responsive-lg nowrap" style="width: 100%">
                                ...
                                <tbody>
                                <?php
                                $tresult = mysqli_query($connection, "SELECT * FROM schedule_detail WHERE schedule_id = '" . $id . "' ORDER BY time_slot");
                                if ($tresult->num_rows == 0) {
                                    ...
                                    while ($trow = mysqli_fetch_assoc($tresult)) { ?>
                                        <tr>
                                            ...
                                            <td><?= $trow["duration"] ?> <small>
                                                    <?php if ($trow["duration"] > 1) echo "hours"; else echo "hour" ?></small>
                                            </td>
                                            <td>
                                                <button data-toggle="modal"
                                                        data-target="#edit<?= $trow['schdetail_id'] ?>"
                                                        class="btn btn-sm btn-outline-info"><i class="fa fa-pen"></i>
                                                    Edit
                                                </button>
                                                <button data-toggle="modal"
                                                        data-target="#delete<?= $trow['schdetail_id'] ?>"
                                        ...
                                        <div class="modal fade" tabindex="-1" role="dialog"
                                             id="edit<?= $trow['schdetail_id'] ?>">
                                            <div class="modal-dialog" role="document">
                                                <div class="modal-content">
                                                    <div class="modal-header">
                                                        <h5 class="modal-title">Edit <?= $trow['time_slot'] ?>
                                                            Schedule</h5>
                                                    ...
                                                    <form action="<?php echo htmlspecialchars($_SERVER["REQUEST_URI"]); ?>"
                                                        ...
                                                        <div class="modal-body">
                                                            <input type="hidden" name="inputSchID"
                                                                   value="<?= $trow["schdetail_id"] ?>">
                                                            <div class="form-group">
                                                                <label for="inputTime">Time Slot</label>
                                                                <input type="text" name="inputEditTime"
                                                                       class="form-control timepicker" id="inputTime"
                                                                       placeholder="Time Slot"
                                                                       value="<?= $trow['time_slot'] ?>">
                                                            </div>
                                                            <div class="form-group">
                                                                <label for="">Duration</label>
                                                                <select name="inputEditDuration"
                                                                        class="form-control <?= $classMin ?>"
                                                                        id="inputMinute">
                                                                    <?php
                                                                    for ($i = 15; $i <= 60; $i += 15) {
                                                                        ?>
...
if (isset($_POST["editbtn"])) {
    ...
    $editstmt = $connection->prepare("UPDATE schedule_detail SET time_slot = ?, duration = ?, status = ? WHERE schdetail_id = ?");
...
if (isset($_POST['deletebtn'])) {
    ...
    $detailstmt = $connection->prepare("DELETE FROM schedule_detail WHERE schdetail_id = ?");
