...
if (isset($_POST["resetbtn"])) {
    ...
    $passstmt = $connection->prepare("SELECT * FROM admin WHERE admin_id =?");
...
?>
    ...
    if (isset($_POST["resetbtn"])) {
        if (count($errors) == 0) {
            ...
            $statement2 = $connection->prepare("UPDATE admin SET admin_pass = ?, admin_token = ? WHERE admin_id = ?");
    ...
    <div class="page-content" id="content">
                        ...
                        <div class="card-body">
                            <div class="form-row">
                                <div class="form-group col-md-6">
                                    ...
                                    <input type="text" name="inputAdminID" class="form-control" id="inputAdminID"
                                           value="<?php echo $admin_row["admin_id"]; ?>" disabled>
                                </div>
                            </div>
                            <div class="form-row">
                                <div class="form-group col-md-6">
                                    ...
                                    <input type="text" name="inputName" class="form-control" id="inputName"
                                           placeholder="Enter Name" value="<?php echo $admin_row["admin_name"]; ?>">
                                </div>
                            </div>
                            <div class="form-row">
                                <div class="form-group col-md-6">
                                    ...
                                    <input type="text" name="inputEmailAddress" class="form-control"
                                           ...
                                           value="<?php echo $admin_row["admin_email"]; ?>">
                                </div>
                            </div>
                        </div>
                    </div>
        ...
        <div class="row">
            <div class="col-12">
                <form name="resetform" method="POST" action="<?php echo htmlspecialchars($_SERVER["REQUEST_URI"]); ?>"
                      autocomplete="off">
                    <div class="card">
                        <div class="card-body">
...
if (isset($_POST["savebtn"])) {
    if (count($errors) == 0) {
        $statement = $connection->prepare("SELECT * FROM admin WHERE admin_email = ?");
        ...
        if ($result->num_rows > 0) {
            echo '<script>
                        Swal.fire({ title: "Error!", text: "Email Address Already Exist!", type: "error" }).then((result) => {
                            if (result.value) { window.location.href = "admin-edit.php?aid=' . $enid . '"; }
                        })
                        </script>';
            exit();
        }
        $statement = $connection->prepare("UPDATE admin SET admin_name = ?, admin_email = ? WHERE admin_id = ? ");
