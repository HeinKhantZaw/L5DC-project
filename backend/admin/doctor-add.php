                            ...
                            <div class="card-body">
                                ...
                                <div class="form-row">
                                    <div class="form-group col-md-6">
                                        ...
                                        <select name="inputClinic" id="inputClinic"
                                                class="form-control selectpicker <?= $classClinic ?>"
                                ...
                                <div class="form-row">
                                    <div class="form-group col-md-6">
                                        ...
                                        <input type="text" name="inputFirstName"
                                               class="form-control <?php echo $classFName ?>" id="inputFirstName"
                                    ...
                                    <div class="form-group col-md-6">
                                        ...
                                        <input type="text" name="inputLastName"
                                               class="form-control <?php echo $classFName ?>" id="inputLastName"
                        ...
                        <div class="card col-md-3">
                            <div class="card-body">
                                <div class="imageupload">
                                    ...
                                    <img src="../assets/img/empty/empty-avatar.jpg" id="output"
                                         class="img-fluid thumbnail <?= $classImage ?>" alt="Doctor-Avatar"
                                         title="Doctor-Avatar">
                                    <div class="file-tab">
                    ...
                    <div class="card">
                        <div class="card-body">
                            <div class="form-group">
                                <label for="inputLanguages">Languages Spoke</label>
                                <div class="row">
                                    <?php $i = 1;
                                        ...
                                        <div class="col">
                                            <div class="custom-control custom-radio custom-control-inline">
                                                <input type="radio" id="inputGenderMale" name="inputGender"
                                                       class="custom-control-input <?= $classGender ?>" value="male">
                                        ...
                                        <div class="col">
                                            <div class="custom-control custom-radio custom-control-inline">
                                                <input type="radio" id="inputGenderFemale" name="inputGender"
                                                       class="custom-control-input <?= $classGender ?>" value="female">
...
if (isset($_POST["savebtn"])) {
    if (multi_empty($errFName, $errLName, $errSpec, $errYears, $errFee, $errSpoke, $errGender, $errEmail, $errContact, $errImage)) {
        ...
        if ($row) {
            echo "<script>Swal.fire('Oops...','Email already exist!','error')</script>";
            exit();
        }
        ...
        $statement = $connection->prepare("INSERT INTO doctors (doctor_avatar, doctor_firstname, doctor_lastname, doctor_speciality, doctor_experience, doctor_desc, doctor_spoke, doctor_gender, doctor_dob, doctor_email, doctor_contact, consult_fee, date_created, clinic_id) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?)");
        $statement->bind_param("ssssissssssssi", $image, $fname, $lname, $speciality, $years, $desc, $spoke, $gender, $dob, $email, $contact, $fees, $date_created, $clinic_id);
        if ($statement->execute()) {
            $selector = bin2hex(random_bytes(8));
            $validator = random_bytes(32);
            $link = $_SERVER["SERVER_NAME"] . "/lifecare/backend/doctor/activate.php?selector=" . $selector . "&validator=" . bin2hex($validator);
            $expries = date("U") + 86400; // one day

            $delstmt = $connection->prepare("DELETE FROM doctor_reset WHERE reset_email = ?");
            $delstmt->bind_param("s", $email);
            $delstmt->execute();

            $hashedToken = password_hash($validator, PASSWORD_DEFAULT);

            $resetstmt = $connection->prepare("INSERT INTO doctor_reset (reset_email, reset_selector, reset_token, reset_expires, activate_token) VALUE (?,?,?,?,?)");
            ...
            if (sendmail($email, $mail['acc_subject'], $mail['acc_title'], $mail['acc_content'], $mail['acc_button'], $link, $token)) {
                ...
                echo '<script>
                Swal.fire({ title: "Something went wrong!", text: "Fail to add the doctor!", type: "error" }).then((result) => {
                    if (result.value) { window.location.href = "doctor-list.php"; }
                });
                </script>';
            }
            ...
            echo '<script>
                Swal.fire({ title: "Something went wrong!", text: "Fail to add the doctor! Please try again", type: "error" }).then((result) => {
                    if (result.value) { window.location.href = "doctor-list.php"; }
                });
                </script>';
        }
