...
if (isset($_POST["savebtn"])) {
    if (count($errors) == 0) {
        ...
        if ($result->num_rows > 0) {
            echo '<script>
            Swal.fire({ title: "Error!", text: "Email exists", type: "error" }).then((result) => {
                if (result.value) { window.location.href = "admin-add.php"; }
            });
            </script>';
            exit();
        }

        $statement = $connection->prepare("INSERT INTO admin (admin_name, admin_email, admin_pass, admin_token, admin_registered) VALUES (?,?,?,?,?) ");
        $statement->bind_param("sssss", $name, $email, $en_pass, $token, $date_created);
