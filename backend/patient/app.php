...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $statement = $connection->prepare("INSERT INTO appointment (app_date, app_time, treatment_type, patient_id, doctor_id, clinic_id) VALUES (?, ?, ?, ?, ?, ?)");
    $statement->bind_param("sssiii", $current_date, $current_time, $treatment, $patient, $doctor, $clinic);
    $statement->execute();
}
?>
<!DOCTYPE html>
...
<form action="" method="POST">
    ...
    <select name="inputPatient" id="">
        ...
        <?php
        $list = $connection->query("SELECT * FROM patients");
        while ($trow = $list->fetch_assoc()):
            echo '<option value="' . $trow['patient_id'] . '">' . $trow['patient_firstname'] . " " . $trow['patient_lastname'] . '</option>';
        endwhile;
        ?>
    </select>

    <label for="">Clinic Branch Name</label>
    <select name="inputClinic" id="">
        <option value="" selected disabled>Choose</option>
        <?php
        $list1 = $connection->query("SELECT * FROM clinics");
    ...
    <select name="inputDoctor" id="">
        <option value="" selected disabled>Choose</option>
        <?php
        $list2 = $connection->query("SELECT * FROM doctors");
...
<div style="margin-top:50px;">
    <table border="1" cellpadding="10">
        ...
        <?php
        $result = $connection->query("SELECT * FROM appointment A JOIN patients P ON A.patient_id = P.patient_id JOIN clinics C ON A.clinic_id = C.clinic_id JOIN doctors D ON A.doctor_id = D.doctor_id");
