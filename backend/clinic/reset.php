...
if (ctype_xdigit($_GET["selector"]) !== false && ctype_xdigit($_GET["validator"]) !== false) {
    ...
    $check = $connection->prepare("SELECT * FROM clinic_reset WHERE reset_selector = ?");
...
if (isset($_POST['resetbtn'])) {
    ...
    if (count($errors) == 0) {
        $currenDate = date("U");
        $statement = $connection->prepare("SELECT * FROM clinic_reset WHERE reset_selector = ? AND reset_expires >= ?");
        ...
        if (!$row = $result->fetch_assoc()) {
            ...
            if ($tokenCheck === false) {
                ...
                $statement = $connection->prepare("SELECT * FROM clinic_staff WHERE staff_email = ?");
                ...
                if (!$row = $result->fetch_assoc()) {
                    ...
                    $updatestmt = $connection->prepare("UPDATE clinic_staff SET staff_password = ?, staff_token = ? WHERE staff_email = ?");
                    $updatestmt->bind_param("sss", $en_pass, $token, $tokenEmail);

                    $delstmt = $connection->prepare("DELETE FROM clinic_reset WHERE reset_email = ?");
