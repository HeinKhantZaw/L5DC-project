...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $forgotstmt = $connection->prepare("SELECT * FROM clinic_staff WHERE staff_email = ?");
            ...
            <div class="login-body">
                <form name="forgot_form" method="POST" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>">
                    ...
                    <div class="form-group">
                        ...
                        <input type="email" name="inputEmailAddress" class="form-control" id="inputEmailAddress"
                               aria-describedby="emailHelp" placeholder="example@email.com">
                        <small id="emailHelp" class="form-text text-muted">Provide us the email id/ mobile of your
                            LifeCare Clinic account<br> We will send you an email with instructions to reset your
                            password.</small>
                    </div>
...
if (isset($_POST["forgotbtn"])) {
    if (count($errors) == 0) {
        ...
        $link = $_SERVER["SERVER_NAME"] . "/lifecare/backend/clinic/reset.php?selector=" . $selector . "&validator=" . bin2hex($validator);
        $expries = date("U") + 1800;

        $userEmail = $_POST["inputEmailAddress"];

        $statement = $connection->prepare("DELETE FROM clinic_reset WHERE reset_email = ?");
        $statement->bind_param("s", $userEmail);
        $statement->execute();

        $hashedToken = password_hash($validator, PASSWORD_DEFAULT);

        $statement = $connection->prepare("INSERT INTO clinic_reset (reset_email, reset_selector, reset_token, reset_expires) VALUE (?,?,?,?)");
        $statement->bind_param("ssss", $userEmail, $selector, $hashedToken, $expries);
        $statement->execute();
